...
class SettingsManager(private val context: Context, sharedPreferences: Lazy<SharedPreferences>) {
    ...
    suspend fun cheatZipFiles() = stringSetPreference(R.string.pref_key_cheat_zip_files, setOf())

    suspend fun addCheatZipFile(uri: String) {
        withContext(Dispatchers.IO) {
            val current = cheatZipFiles()
            sharedPreferences.getStringSet(getString(R.string.pref_key_cheat_zip_files), setOf())
                .set(current + uri)
        }
    }

    suspend fun removeCheatZipFile(uri: String) {
        withContext(Dispatchers.IO) {
            val current = cheatZipFiles()
            sharedPreferences.getStringSet(getString(R.string.pref_key_cheat_zip_files), setOf())
                .set(current - uri)
